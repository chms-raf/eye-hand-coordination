...
#define MAXLINE 1024
#define BUFLEN 64
#define DSPACE_IP "137.148.205.222"          // dSPACE
...
int main( )
{
    ...
    while ( !glfwWindowShouldClose( window ) )
    {
        ...
        transform = glm::translate( transform, glm::vec3( 0.0f, -0.30f, 0.0f ) );
...
{
    if (handX < transformedLocationInitial[0] + xEdge*2 && handX > transformedLocationInitial[0] -xEdge*2 && handY < transformedLocationInitial[1] + yEdge*2 && handY > transformedLocationInitial[1] -yEdge*2)
    {
    ...
    if ( handX < transformedLocationFinal[0] + xEdge*2 && handX > transformedLocationFinal[0] - xEdge*2 && handY < transformedLocationFinal[1] + yEdge*2 && handY > transformedLocationFinal[1] - yEdge*2)
    {
    ...
    if (eyeX < transformedLocationInitial[0] + xEdge*7 && eyeX > transformedLocationInitial[0] -xEdge*7 && eyeY < transformedLocationInitial[1] + yEdge*7 && eyeY > transformedLocationInitial[1] -yEdge*7)
    {
        whiteSquareEye = true;
    }
    else
    ...
    if ( eyeX < transformedLocationFinal[0] + xEdge*7 && eyeX > transformedLocationFinal[0] - xEdge*7 && eyeY < transformedLocationFinal[1] + yEdge*7 && eyeY > transformedLocationFinal[1] - yEdge*7)
    {
        greenSquareEye = true;
    }
    else
...
void whatStep()
{
    switch (temp) {
        ...
        case 1:                             // Subject eyes are stable within tolerance zone and hand remain on white square for 1 second
            if (timer >= 0.5)               // If eyes are stable within tolerance zone and hand is within the bounds of the white square for more than 1 second
            {
                //printf("\007");
