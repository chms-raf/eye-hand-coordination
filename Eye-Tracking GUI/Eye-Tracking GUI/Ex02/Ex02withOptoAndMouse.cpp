...
double udpData[9];                          // UDP packet to send (time, target, POG, opto)
...
void Receive_UDP2(void)
{
    ...
    /*
     ...
     double slopeX = 1.0 * (WIDTH - 0) / (47.5625 - 0);     // Slope for mapping POG X coordinates to screen coordinates (inches)
     double slopeY = 1.0 * (HEIGHT - 0) / (25.5625 - 0);    // Slope for mapping POG Y coordinates to screen coordinates (inches)
     double outputX = 0 + round(slopeX * (POG[0] - 0));
     double outputY = 0 + round(slopeY * (POG[1] - 0));
...
void Send_UDP(void)
{
    ...
    udpData[1] = transformedLocationFinal[0];
    udpData[2] = transformedLocationFinal[1];
    udpData[3] = POG[0];
    udpData[4] = POG[1];
    udpData[5] = POG[2];
    udpData[6] = optoData[0];
    udpData[7] = optoData[1];
    udpData[8] = optoData[2];
    ...
    results.reserve(g1.size() + g2.size() + g3.size() + g4.size() + g5.size() + g6.size() + g7.size() + g8.size() + g9.size());
...
int main( )
{
    ...
    while ( !glfwWindowShouldClose( window ) )
    {
        ...
        Send_UDP();
        //printf("Sent Data - Time: %f, Target X: %f, Target Y: %f, POGx: %f, POGy: %f, POGz: %f, optoX: %f, optoY: %f, optoZ: %f\n", currentTime, transformedLocationFinal[0], transformedLocationFinal[1], POG[0], POG[1], POG[2], optoData[0], optoData[1], optoData[2]);
    }
...
void whatStep()
{
    switch (temp) {
        case 0:                         // White square appears. Eye and hand move to white square
            if (whiteSquareEye && whiteSquareHand)        // If eye and hand are within bounds of the white square
            {
        ...
        case 1:                             // Subject eye and hand remain on white square for 1 second
            if (glfwGetTime() >= 1.0)       // If eye and hand are within the bounds of the white square for more than 1 second
            {
            ...
            if (!whiteSquareHand || !whiteSquareEye)               // If eye or hand leave bounds of white square before 1 second has been reached
            {
                temp = 0;
        ...
        case 2:                            // Red square appears. Eye only moves to red square
            if (greenSquareEye)            // If the subject's gaze is within the bounds of the red square
            {
                glfwSetTime(0);             // reset timer
                temp = 3;
            }
        ...
        case 3:                             // Subject eyes on red square for 1 second
            ...
            if (!greenSquareEye)            // If eye leaves the bounds of the red square before 1 seconds has been reached
            {
                glfwSetTime(0);             // reset timer
            }
            break;
        ...
        case 4:                             // Red square changes to green. Hand moves to green square. Eye stays on green square
            if (greenSquareHand && greenSquareEye)
            {
                glfwSetTime(0);             // reset timer
                temp = 5;
            }
