...
#define DSPACE_IP "137.148.205.30"          // IP address of dSPACE

// Window Dimension Definitions
// Test window: 800 X 500
// Full screen Macbook: 1440 X 855
// Full screen samsung TV: 1920 X 1032
#define SCREEN_WIDTH 1440                   // Width of openGL window
#define SCREEN_HEIGHT 855                   // Height of openGL window
...
// Global Variables
float defaultX = SCREEN_WIDTH / 2;          // X default position of fixation target (screen coordinates)
float defaultY = SCREEN_HEIGHT * 0.2;       // Y default position of fixation target (screen coordinates)
float X = defaultX;                         // X position of fixation target (screen coordinates)
float Y = defaultY;                         // Y position of fixation target (screen coordinates)
...
double X_mm;                                // X position of target (mm)
double Y_mm;                                // Y position of target (mm)
double eyeX_Screen;                         // X position of eyes (screen coordinates)
double eyeY_Screen;                         // Y position of eyes (screen coordinates)
double handX_Screen;                        // Y position of hand (screen coordinates)
double handY_Screen;                        // Y position of hand (screen coordinates)
...
static float targets [numTargets][2]= {
    {0.2 * SCREEN_WIDTH, defaultY},         // Target 1
    {0.3 * SCREEN_WIDTH, defaultY},         // Target 2
    {0.7 * SCREEN_WIDTH, defaultY},         // Target 3
    {0.8 * SCREEN_WIDTH, defaultY},         // Target 4
    {defaultX, 0.4 * SCREEN_HEIGHT},        // Target 5
    {defaultX, 0.7 * SCREEN_HEIGHT}         // Target 6
};
...
int main( void )
{
    ...
    while ( !glfwWindowShouldClose( window ) )  // Will run until the window is closed
    {
        ...
        if (startVal) {                         // Program will start when the ENTER key is pressed
            ...
            //Receive_UDP_Eye();
            //Receive_UDP_Hand();
            //printf("HandX: %f, HandY: %f, HandZ: %f\n", handX, handY, handZ);
            ...
            drawPlus(X, Y, 0);
            if (state == 0) {
                glColor3f(0.0f, 1.0f, 0.0f);    // change draw color to white
            }
            else {
            ...
            // Map hand (optotrak coordinates) to screen coordinates
            handX_Screen = linearMap(0, SCREEN_WIDTH, 0, 1210, handX);
            handY_Screen = linearMap(0, SCREEN_HEIGHT, 0, 651, handY);
            
            //handX_Screen = linearMap(SCREEN_WIDTH, 1210, handX);
            //handY_Screen = linearMap(SCREEN_HEIGHT, 651, handY);
            
            // Draw feedback cursor
            drawCursor(eyeX_Screen, eyeY_Screen, 0, SCREEN_WIDTH, 1);     //Provides eye feedback
            //drawCursor(handX_Screen, handY_Screen, 0, 10, 1);     //Provides hand feedback
            
            // Map target (screen coodintates) to mm
            X_mm = linearMap(0, SCREEN_WIDTH, 0, 1210, X);
            Y_mm = linearMap(0, SCREEN_HEIGHT, 0, 651, Y);
            
            //X_mm = linearMap(1210, SCREEN_WIDTH, X);
            //Y_mm = linearMap(651, SCREEN_HEIGHT, Y);
...
void Receive_UDP_Eye(void)
{
    ...
    /*
    double slopeX = 1.0 * (SCREEN_WIDTH - 0) / (511 - 18);
    double outputX = slopeX * (destination[0] - 18);
    eyeX_Screen = outputX + 40;
    
    double slopeY = 1.0 * (0 - SCREEN_HEIGHT) / (467 - 18);
    double outputY = slopeY * (destination[1] - 467);
    eyeY_Screen = outputY;
    //printf("outputY: %f, outputY2: %f\n", outputY, outputY2);
     */
    ...
    //eyeX = linearMap(1210, 511, destination[0]);
    //eyeY = inverselinearMap(651, 511, destination[1]);
...
//-----------------------------------------------------------------------
/*
double linearMap(double x1, double x2, double y1, double y2, double x)
{
    double slope = 1.0 * (y2 - y1) / (x1 - x2);
    double output = slope * (x - x2);
...
//-----------------------------------------------------------------------
/*
double inverselinearMap(double x1, double x2, double value)
{
    double slope = 1.0 * (x1 - 0) / (0 - x2);
    double output = 0 + round(slope * (value - x2));
