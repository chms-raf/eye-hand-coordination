...
int main( )
{
    ...
    while ( !glfwWindowShouldClose( window ) )
    {
        ...
        switch (temp) {
            ...
            case 1:
                if (glfwGetTime() >= 1.0)       // If cursor is within the bounds of the white square for more than 2 seconds
                {
                    state = 1;
                    temp = 2;
...
static void cursorPositionCallback( GLFWwindow *window, double xPos, double yPos)
{
    /*
     ...
     // Detect where cursor is
     if (X < transformedLocationInitial[0] + xEdge && X > transformedLocationInitial[0] -xEdge && Y < transformedLocationInitial[1] + yEdge && Y > transformedLocationInitial[1] -yEdge)
     {
     whiteSquare = true;
     }
     else
